...
LLM_ASSERTION = RootCauseHypothesis(
    ...
    fix_hint=(
        "Review get_balance logic for user_id=7 and verify the test fixture "
        "creates the correct balance."
    ),
    confidence=0.95,
    is_flaky=False,
)
...
LLM_IMPORT = RootCauseHypothesis(
    ...
    fix_hint=(
        "Run `pip install redis` or add redis to pyproject.toml dev dependencies "
        "and reinstall."
    ),
    confidence=0.98,
    is_flaky=False,
...
LLM_FIXTURE = RootCauseHypothesis(
    ...
    explanation=(
        "pytest cannot locate the 'db_session' fixture. It may be missing from conftest.py."
    ),
    fix_hint=(
        "Add db_session fixture to tests/conftest.py and ensure conftest.py is in the "
        "correct directory."
    ),
    confidence=0.92,
    is_flaky=False,
...
LLM_TIMEOUT = RootCauseHypothesis(
    ...
    explanation=(
        "The test_slow_query test timed out after 5 seconds, suggesting a missing "
        "index or blocking query."
    ),
...
LLM_NETWORK = RootCauseHypothesis(
    ...
    explanation=(
        "The test makes a real network call that failed. External services should "
        "be mocked in tests."
    ),
    fix_hint=(
        "Mock the requests.get call with unittest.mock or responses library to "
...
LLM_ENVIRONMENT = RootCauseHypothesis(
    ...
    fix_hint=(
        "Add SMTP_HOST to .env.example and set it in the CI environment or use "
...
LLM_DATA = RootCauseHypothesis(
    ...
    fix_hint=(
        "Create tests/fixtures/config.json or update the path in the test to "
...
LLM_FLAKY = RootCauseHypothesis(
    ...
    explanation=(
        "The test asserts 100 results but gets 99 due to a race condition in "
        "the writer pool."
    ),
    fix_hint=(
        "Add proper locking around the writer pool or use pytest-xdist isolation "
